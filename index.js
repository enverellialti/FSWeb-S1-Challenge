...
function indekseGoreFenomen(fenomenler, j = 3) {

  return (`${j}. indekste bulunan fenomen: ${fenomenler[j].profile}`);
}
...
function profilListesi(fenomenler) {
  var fenomenKopya = [...fenomenler];
  var fenomenAdlari = fenomenKopya.map((fenomen) => fenomen.profile);
  return fenomenAdlari
}
...
function fenomenSil(fenomenler, k = 2) {
  var fenomenKopya = [...fenomenler];
  fenomenKopya.splice(k, 1);
  return fenomenKopya;
...
function fenomenEkle(fenomenler, p2 = 6, p3 = "Workintech", p4 = 10000000, p5 = 2022, p6 = "Instagram") {
  var fenomenKopya = [...fenomenler];
  var oNew = {
    "number": 6,
    "profile": "Workintech",
...
function enFenomenler(fenomenler) {
  var fenomenCokTakipcili = fenomenler.filter(fenomenler => fenomenler.followers > 100000000);
  var fenomenCokTakipciliAdlari = fenomenCokTakipcili.map((feno) => feno.profile);
  return fenomenCokTakipciliAdlari;
}
...
function fenomenGonderimSayisi(i = fenomenler, j = ' Will Smith') {
  var fenomen = fenomenler.find(fenomenler => fenomenler.profile == 'Will Smith');
  if (fenomen) {
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, k) {

  var fenoPlatform = [];
  var fenoMax = 0;
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].platform == k && fenomenler[i].posts > fenoMax && fenomenler[i].posts !== "NA") {
      fenoMax = fenomenler[i].posts;
      fenoPlatform.push(fenomenler[i]);
    }
  }
  return fenoPlatform[fenoPlatform.length - 1].profile;
}
